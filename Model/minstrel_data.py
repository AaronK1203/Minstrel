...
class FileWriter:
    ...
    def __init__(self,file):
        self.file = file
        self.df = pd.read_csv(f'../Saving/Playlists/{self.file}',
                              names = ['name','path','author','intervals','bpm','total_time','category'])
    def save_df(self,df):
        df.to_csv('f../Saving/Playlists/{self.file}',index = False,header = False)
...
class MusicArray:
    ...
    def query_to_list(self,name,category,**kwargs):
        ...
        if gate == 'or':
            ...
            #Ensure that 
            if name == '':
                name = 'Somethingyouwillneverfindinaname854058qbqwbjkbdsjkbejbd'
            if category == '':
                category = 'thiswillneverbeacategory478695jwenjejksejkbj'
            for song in self.array:
